...
public class ConfigHandler {
	...
	public ConfigHandler(Money money) {
		this.money = money;
		if (!(new File("plugins" + System.getProperty("file.separator") + "MysqlEconomyBank"
				+ System.getProperty("file.separator") + "config.yml").exists())) {
			Money.log.info("No config file found! Creating new one...");
			money.saveDefaultConfig();
		}
		try {
			money.getConfig().load(new File("plugins" + System.getProperty("file.separator") + "MysqlEconomyBank"
	...
	public String getString(String key) {
		if (!money.getConfig().contains(key)) {
			money.getLogger().severe("Could not locate '" + key
					+ "' in the config.yml inside of the MysqlEconomyBank folder! (Try generating a new one by deleting the current)");
	...
	public void printMessage(Player player, String messageKey, String amount, Player player2, String player2Name) {
		if (money.getConfig().contains(messageKey)) {
			...
			if (player2 != null && money.getMoneyDatabaseInterface().hasAccount(player2)) {
				if (money.getMoneyDatabaseInterface().getBalance(player2).toString().endsWith(".0")) {
					DecimalFormat fr = new DecimalFormat("#,##0");
					message.set(0, message.get(0).replaceAll("%balance",
							"" + fr.format(money.getMoneyDatabaseInterface().getBalance(player2))));
				} else
					message.set(0, message.get(0).replaceAll("%balance",
							"" + f.format(money.getMoneyDatabaseInterface().getBalance(player2))));
			} else {
			...
			if (player != null) {
				// Message format
				player.sendMessage(
						parseFormattingCodes(getString("chatMessages.prefix")) + parseFormattingCodes(message.get(0)));
			...
			money.getLogger().severe(
					"Could not locate '" + messageKey + "' in the config.yml inside of the MysqlEconomyBank folder!");
			player.sendMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + ">> " + ChatColor.RED + "Could not locate '"
					+ messageKey + "' in the config.yml inside of the MysqlEconomyBank folder!");
		}
	}
	...
	public void actionBarMessage(Player player, String messageKey) {
		if (money.getConfig().contains(messageKey)) {
			...
			if (money.getMoneyDatabaseInterface().getBalance(player).toString().endsWith(".0")) {
				DecimalFormat fr = new DecimalFormat("#,##0");
				message.set(0, message.get(0).replaceAll("%bankBalance",
						"" + fr.format(money.getMoneyDatabaseInterface().getBalance(player))));
			} else {
				message.set(0, message.get(0).replaceAll("%bankBalance",
						"" + f.format(money.getMoneyDatabaseInterface().getBalance(player))));
			}

			if (Double.toString(Money.econ.getBalance(player)).endsWith(".0")) {
				DecimalFormat fr = new DecimalFormat("#,##0");
				message.set(0,
						message.get(0).replaceAll("%pocketBalance", "" + fr.format(Money.econ.getBalance(player))));
			} else {
				message.set(0,
						message.get(0).replaceAll("%pocketBalance", "" + f.format(Money.econ.getBalance(player))));
			}
