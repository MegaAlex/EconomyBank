				...
				if (target != null) {
					if (target.isOnline()) {
						if (money.getMoneyDatabaseInterface().hasAccount(target) == false) {
							money.getConfigurationHandler().printMessage(((Player) sender).getPlayer(),
									"chatMessages.accountDoesNotExist", "0", target, target.getName());
						...
						try {
							...
							money.getConfigurationHandler().printMessage(target, "chatMessages.setCommand",
									amount.toString(), target, target.getName());
							money.getSoundHandler().sendClickSound(p);
							return true;
						} catch (Exception e) {
							money.getConfigurationHandler().printMessage(((Player) sender).getPlayer(),
									"chatMessages.setCommandFail", "0", null, null);
							money.getSoundHandler().sendPlingSound(p);
							return false;
						}
					...
					try {
						...
						if (money.getMoneyDatabaseInterface().hasAccount(targetUUID) == false) {
							money.getConfigurationHandler().printMessage(((Player) sender).getPlayer(),
									"chatMessages.accountDoesNotExist", "0", null, "null");
							money.getSoundHandler().sendPlingSound(p);
							return false;
						}
						...
						try {
							...
							money.getConfigurationHandler().printMessage(((Player) sender).getPlayer(),
									"chatMessages.setCommand", amount.toString(), null, "null");
						...
						money.getConfigurationHandler().printMessage(((Player) sender).getPlayer(),
								"chatMessages.balanceCommandFail", "0", null, null);
						money.getSoundHandler().sendPlingSound(p);
						return false;
					}
				}
				...
				if (target.isOnline()) {
					...
					try {
						...
						sender.sendMessage(ChatColor.GREEN + ">> " + ChatColor.WHITE + "" + target.getName()
								+ ChatColor.GREEN + " balance set to: " + ChatColor.WHITE + "" + amount);
						return true;
					} catch (Exception e) {
				...
				try {
					...
					try {
						...
						sender.sendMessage(ChatColor.GREEN + ">> " + ChatColor.WHITE + "" + targetUUID + ChatColor.GREEN
								+ " balance set to: " + ChatColor.WHITE + "" + amount);
						return true;
					} catch (Exception e) {
						sender.sendMessage(ChatColor.RED + ">> The amount must be a number!");
						return false;
