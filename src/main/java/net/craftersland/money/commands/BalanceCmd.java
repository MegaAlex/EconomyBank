		...
		if (sender instanceof Player) {
			...
			if (p.hasPermission("MysqlEconomyBank.cmd.balance")) {
				if (money.cooldown.contains(p.getUniqueId())) {
					money.getConfigurationHandler().printMessage(p, "chatMessages.tooFastInteraction", "0", p,
							p.getName());
				...
				Double delayCalc = 20.00 / 1000.00 * Double
						.parseDouble(money.getConfigurationHandler().getString("general.timeBetweenTwoInteractions"));
	...
	public boolean runAdminCmd(CommandSender sender, String[] args) {
		if (sender instanceof Player) {
			...
			if (p.hasPermission("MysqlEconomyBank.admin")) {
				...
				UUID targetId = UUID.nameUUIDFromBytes(("OfflinePlayer:" + targetName.toLowerCase()).getBytes());
				try {
					if (!money.getMoneyDatabaseInterface().hasAccount(targetId)) {
						money.getConfigurationHandler().printMessage((Player) sender,
								"chatMessages.accountDoesNotExist", "0", null, targetName);
						money.getSoundHandler().sendPlingSound(p);
						return false;
					}
					String amount = money.getMoneyDatabaseInterface().getBalance(targetId).toString();
					money.getConfigurationHandler().printMessage((Player) sender, "chatMessages.balanceCommand", amount,
							null, targetName);
					money.getSoundHandler().sendClickSound(p);
					return true;
				} catch (Exception e) {
					money.getConfigurationHandler().printMessage((Player) sender, "chatMessages.balanceCommandFail",
							"0", null, targetName);
					money.getSoundHandler().sendPlingSound(p);
					return false;
				}
			...
			if (target != null) {
				if (target.isOnline()) {
					if (money.getMoneyDatabaseInterface().hasAccount(target) == false) {
						sender.sendMessage(ChatColor.RED + ">> " + ChatColor.WHITE + "" + target.getName()
								+ ChatColor.RED + " does not have an account!");
						return false;
					}
					String amount = money.getMoneyDatabaseInterface().getBalance(target).toString();
					sender.sendMessage(ChatColor.GREEN + ">> " + ChatColor.WHITE + "" + target.getName()
							+ ChatColor.GREEN + " balance: " + ChatColor.WHITE + "" + amount);
					return true;
				}
			} else {
				try {
					...
					if (money.getMoneyDatabaseInterface().hasAccount(targetUUID) == false) {
						sender.sendMessage(ChatColor.RED + ">> " + ChatColor.WHITE + "" + targetUUID + ChatColor.RED
								+ " does not have an account!");
						return false;
					}
					String amount = money.getMoneyDatabaseInterface().getBalance(targetUUID).toString();
					sender.sendMessage(ChatColor.GREEN + ">> " + ChatColor.WHITE + "" + targetUUID + ChatColor.GREEN
							+ " balance: " + ChatColor.WHITE + "" + amount);
					return true;
				} catch (Exception e) {
