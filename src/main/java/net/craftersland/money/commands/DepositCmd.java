			...
			if (p.hasPermission("MysqlEconomyBank.cmd.deposit")) {
				...
				if (Money.econ.getBalance(p) >= amount) {
					...
					if (bankBalance + amount > Double
							.parseDouble(m.getConfigurationHandler().getString("general.maxBankLimitMoney"))) {
						m.getConfigurationHandler().printMessage(p, "chatMessages.reachedMaximumMoneyInAccount",
								amount + "", p, p.getName());
						m.getSoundHandler().sendPlingSound(p);
						return true;
					}
					...
					m.getConfigurationHandler().printMessage(p, "chatMessages.depositedSuccessfully", amount + "", p,
							p.getName());
