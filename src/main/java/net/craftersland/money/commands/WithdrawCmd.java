			...
			if (p.hasPermission("MysqlEconomyBank.cmd.withdraw")) {
				...
				if (bankBalance >= amount) {
					if (Money.econ.getBalance(p) + amount > Double
							.parseDouble(m.getConfigurationHandler().getString("general.maxPocketLimitMoney"))) {
						m.getConfigurationHandler().printMessage(p, "chatMessages.reachedMaximumMoneyInPocket",
								amount + "", p, p.getName());
						m.getSoundHandler().sendPlingSound(p);
						return true;
					}
					...
					m.getConfigurationHandler().printMessage(p, "chatMessages.withdrewSuccessfully", amount + "", p,
							p.getName());
					...
					Double delayCalc = 20.00 / 1000.00 * Double
							.parseDouble(m.getConfigurationHandler().getString("general.timeBetweenTwoInteractions"));
				...
				m.getConfigurationHandler().printMessage(p, "chatMessages.notEnoughMoneyInAccount", amount + "", p,
						p.getName());
				m.getSoundHandler().sendPlingSound(p);
				return true;
			} else {
