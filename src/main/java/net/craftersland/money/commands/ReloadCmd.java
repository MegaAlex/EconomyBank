	...
	public boolean runCmd(CommandSender sender) {
		if (sender instanceof Player) {
			...
			if (p.hasPermission("MysqlEconomyBank.admin")) {
				try {
					money.getConfig().load(new File("plugins" + System.getProperty("file.separator")
							+ "MysqlEconomyBank" + System.getProperty("file.separator") + "config.yml"));
				} catch (Exception e) {
			...
			try {
				...
				sender.sendMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + ">> " + ChatColor.RED
						+ "Could not load config! Check logs!");
				e.printStackTrace();
				return false;
			}
			sender.sendMessage(
					ChatColor.DARK_GREEN + "" + ChatColor.BOLD + ">> " + ChatColor.GREEN + "Configuration reloaded!");
			return true;
		}
	}
