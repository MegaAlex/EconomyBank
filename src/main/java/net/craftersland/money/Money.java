...
public final class Money extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (cH.getString("database.typeOfDatabase").equalsIgnoreCase("mysql")) {
			...
			if (!new File("plugins" + System.getProperty("file.separator") + "MysqlEconomyBank"
					+ System.getProperty("file.separator") + "Accounts").exists()) {
				(new File("plugins" + System.getProperty("file.separator") + "MysqlEconomyBank"
						+ System.getProperty("file.separator") + "Accounts")).mkdir();
			}
	...
	private boolean getMcVersion() {
		...
		if (version.matches("1.7.10") || version.matches("1.7.9") || version.matches("1.7.5")
				|| version.matches("1.7.2") || version.matches("1.8.8") || version.matches("1.8.7")
				|| version.matches("1.8.3") || version.matches("1.8.4") || version.matches("1.8")) {
			is19Server = false;
			return true;
		} else {
			log.warning("Possible incompatible server version detected: " + version
					+ " .Attempting to run in 1.9 and 1.10 compatibility mode! If you have errors report them.");
		}
		return false;
	}
	...
	public void interestTask() {
			...
			public void run() {
				...
				if (!onlinePlayers.isEmpty()) {
					for (Player p : onlinePlayers) {
						// if (econ.get)
						Double intPercentage = Double.parseDouble(getConfigurationHandler()
								.getString("general.interest.percentageAmount").replace("%", ""));
						...
						if (balance < getConfigurationHandler().getInteger("general.maxBankLimitMoney")) {
							...
							getConfigurationHandler().printMessage(p, "chatMessages.interest", interest.toString(), p,
									p.getName());
						}
					}
		...
		}.runTaskTimerAsynchronously(this, getNextInterestTaskTime() * 20L, time * 1200L);
	}
	...
	public long getNextInterestTaskTime() {

		long intervalTime = getConfigurationHandler().getInteger("general.interest.interestTime") * 60;
		long curT = System.currentTimeMillis() / 1000L;
		long nextRun = ((((curT - 300) / intervalTime) + 1) * intervalTime + 300L) - curT;
		return nextRun;
	}
